...
## 🆕 Latest Discovery: iTunes Charts V2 API

**New MZStoreServices Charts Endpoint Discovered!**

We've discovered a new iTunes API endpoint that provides access to additional chart types:

```
https://itunes.apple.com/WebObjects/MZStoreServices.woa/ws/charts?cc=us&g=36&name=FreeAppsV2&limit=100
```

### Key Features:
- **12+ Chart Types**: Including `FreeAppsV2`, `PaidApplications`, `AppsByRevenue`, and more
- **Higher Limits**: Up to 200 app IDs per request (vs 200 max from legacy API)
- **Simpler Parameters**: Named chart types instead of numeric IDs
- **Better Structure**: Clean JSON response with `resultIds` array

### Available in Web Demo:
- **New Endpoint**: `POST /api/charts-v2` - Uses the new MZStoreServices API
- **Chart Types**: `GET /api/chart-types` - Lists all available chart types
- **Full Documentation**: See `CHARTS_V2_API.md` for complete usage guide
